...
function GetCompany_DDL() {
    new APICALL(GetGlobalURL('Base', 'GetCompanyDDL'), 'GET', '', true).FETCH((result, error) => {

        if (result) {
            if (result.data != null) {
                $.each(result.data, function (i, option) {
                    $('#companyId').append(
                        '<option value="' + option.CompanyId + '">' + option.CompanyDesc + '</option>'
                    );
                });
            }
        }
