...
function ViewData() {
    ...
    UTILITY.CheckSession((data_) => {
        if (data_) {
            new APICALL(GetGlobalURL('Base', 'GetCurrency'), 'GET', '', true).FETCH((result, error) => {
                if (result) {
                    ...
                    if (result.data != null) {
                        ...
                        $.each(result.data, function (i, option) {
                            var primaryID = option.CurrencyId
                            ...
                            $('#user-master tbody').append(
                                '<tr id="rowid-' + i + '">' +
                                '<td>' + option.CurrencyCode + '</td>' +
                                '<td>' + option.CurrencyDesc + '</td>' +
                                '<td>' + option.CurrencyAbbr + '</td>' +
                                '<td>' + option.CurrencyRate + '</td>' +
                                '<td>' + baseCurrency + '</td>' +
                                '<td>' + activeBadge + '</td>' +
                                UpdateBtn +
                                DeleteBtn +
