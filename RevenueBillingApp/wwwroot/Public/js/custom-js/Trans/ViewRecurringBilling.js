...
function ViewData() {
    ...
    UTILITY.CheckSession((data_) => {
        if (data_) {
            new APICALL(GetGlobalURL('Base', 'GetViewAllRecurringBillingForm'), 'GET', '', true).FETCH((result, error) => {
                if (result) {
                    ...
                    if (result.data != null) {
                        ...
                        $.each(result.data, function (i, option) {
                            var primaryID = option.rbilling_id;
                            ...
                            var bill_start_date = option.bill_start_date
                                ? option.bill_start_date.split('T')[0]
                                : '-';

                            var bill_end_date = option.bill_end_date
                                ? option.bill_end_date.split('T')[0]
                                : '-';

                            var due_date = option.due_date
                                ? option.due_date.split('T')[0]
                                : '-';
                            ...
                            $('#user-master tbody').append(
                                ...
                                '<td>' + (option.tax || '0') + '</td>' +
                                '<td>' + (option.total_amount || '0') + '</td>' +
                                '<td>' + (due_date || '') + '</td>' +
                                '<td>' + (option.billing_status || '') + '</td>' +
                                UpdateBtn +
                                DeleteBtn +
                                '</tr>'
                            );
...
function EditData(id) {
    window.location.href =
        "/Trans/RecurringBilling?RBillingId=" + id;
}
function DeleteData(ID) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            new APICALL(GetGlobalURL('Base', 'DeleteRecurringBillingForm'), 'POST', DeleteData, true).FETCH((result, error) => {
                console.log(result);
