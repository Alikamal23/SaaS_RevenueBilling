...
$(document).ready(function () {
    ...
    GetVoucherTypeDDL("0"); //Debit or Payment Voucher
    ...
    $('#btnAddNew').on('click', function () {
        $('#myModalLabel').html('Add Debit Voucher');
...
function ViewData() {
    ...
    UTILITY.CheckSession((data_) => {
        if (data_) {
            new APICALL(GetGlobalURL('Base', 'GetVoucherMaster?Nature=0'), 'GET', '', true).FETCH((result, error) => {
                if (result) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $("#DetailSectionID tbody tr").each(function () {
                ...
                ////detailSection.DebitAmount = parseFloat(row.find("td").eq(8).html());
                detailSection.DebitAmount = parseFloat(row.find("td").eq(9).html().replace(/,/g, '')); // FIXED
            ...
            var data = {
                ...
                VDate: $("#txtVDate").val(),
                VNo: '',
                Folio: 'DR',  //for Debit or Credit Voucher
                ModeOfPayment: '',
                BankName: $("#ddlCrHead option:selected").text(),
                Chq_PO_Draft_No: '',
                CashType: cashType,
                Narration: $("#txtNarration").val(),
                DCCategory: $("#txtMasterCategoryId").val(),
                DCCode: $("#ddlCrHead").val(),
                Amount: amount,
...
function EditData(ID, index) {
    $('#myModalLabel').html('Edit Debit Voucher');
