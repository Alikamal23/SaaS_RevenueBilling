...
function Show_Report_Preview(obj) {
    ...
    new APICALL(GetGlobalURL('Base', 'GetClientInvoiceReport'), 'POST', JSON.stringify(obj), true).FETCH((result, error) => {
        if (error) {
            Swal.fire({
                icon: 'error',
                title: 'Error...',
        ...
        if (result && result.data) {
            ...
            if (obj.ShowExcel) {
                new JSONTOEXCEL().Export(result.data, "Client Invoice Report", "Report", []);
